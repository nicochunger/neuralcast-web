...
export function AdminListeners() {
  ...
  return <div className="adminAudience">
    ...
    <p className="adminReportStatus" role="status">{error || (report ? `${range === "live" ? "Live report · Refreshes every 15 seconds" : "Historical report · Unique listeners within the selected period"} · Updated ${new Date(report.fetchedAt).toLocaleTimeString()}` : "Connecting to AzuraCast…")}{error && report ? " Showing the last successful report; retrying automatically." : ""}</p>
    {report ? <>
